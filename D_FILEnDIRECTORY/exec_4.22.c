...
{
	fullpath = path_alloc(&pathlen);	/* malloc으로 PATH_MAX+1 bytes를 할당 */
...
dopath(Myfunc* func)
{
	...
	if (S_ISDIR(statbuf.st_mode) == 0) /* not directory */
		return(func(fullpath, &statbuf, FTW_F));

	/*
	* 디렉터리이면 먼저 디렉터리 자체에 대해 func()을 호출한 후
	...
	*/
	...
	while ((dirp = readdir(dp)) != NULL) {
		...
		strcpy(&fullpath[n], dirp->d_name);	/* 이름뒤에 "/"를 추가 */
		if ((ret = dopath(func)) != 0)		/* 재귀 호출 */
			break;
	} 
	fullpath[n-1] = 0; /* "/" 이후의 모든 것을 삭제 */
...
myfunc(const char *pathname, const struct stat *statptr, int type)
{
	...
	case FTW_F:
		...
		case S_IFDIR: nreg++;	/* if directory, type must be FTW_D */
